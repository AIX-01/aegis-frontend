...
function CameraInfo({
  ...
  return (
    <div className="flex items-center gap-1.5">
      <Camera className="h-3.5 w-3.5 text-white flex-shrink-0 icon-shadow" />
      <div className="flex flex-col min-w-0">
        <div className="flex items-center gap-1">
          <span className="text-xs font-medium text-white truncate text-shadow">{camera.alias}</span>
          {showEdit && onEditClick && (
            <Button size="icon" variant="ghost" className="h-5 w-5 text-white hover:bg-white/10" onClick={onEditClick}>
              <Pencil className="h-2.5 w-2.5 text-white icon-shadow" />
            </Button>
          )}
        </div>
        <span className="text-[10px] font-mono text-white/80 truncate text-shadow-sm">{camera.name}</span>
      </div>
    </div>
  );
}
      ...
      {isModalOpen && selectedCamera && (
        <div className="fixed inset-0 z-50 bg-black">
          ...
          <div className="absolute top-3 left-3 z-20 flex items-center gap-2">
            ...
            {isEditingAlias ? (
              <div className="flex items-center gap-1.5">
                <Camera className="h-3.5 w-3.5 text-white icon-shadow" />
                <div className="flex flex-col">
                  <div className="flex items-center gap-1">
                    ...
                    <Button size="icon" variant="ghost" className="h-6 w-6 text-white hover:bg-white/10" onClick={handleSaveAlias}>
                      <Check className="h-3 w-3 text-success icon-shadow" />
                    </Button>
                    <Button size="icon" variant="ghost" className="h-6 w-6 text-white hover:bg-white/10" onClick={handleCancelEdit}>
                      <X className="h-3 w-3 icon-shadow" />
                    </Button>
                  </div>
                  <span className="text-[10px] font-mono text-white/80 text-shadow-sm">{selectedCamera.name}</span>
                </div>
              </div>
            ) : (
          ...
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3 bg-black/40 border border-white/20 rounded-lg px-3 py-2">
            <div className="flex items-center gap-2 text-xs text-white">
              <Power className="h-3.5 w-3.5 text-white icon-shadow" />
              <span className="text-white text-shadow-sm">카메라</span>
              <Switch
            ...
            <div className={cn("flex items-center gap-2 text-xs text-white", !selectedCamera.enabled && "opacity-50")}>
              <Brain className="h-3.5 w-3.5 text-white icon-shadow" />
              <span className="text-white text-shadow-sm">AI 분석</span>
              <Switch
          ...
          {!selectedCamera.connected && (
            <div className="absolute inset-0 flex items-center justify-center z-[5]">
              <div className="text-center px-6 py-4 rounded-lg border border-white/30 bg-black/40">
                <WifiOff className="h-12 w-12 text-white mx-auto mb-2 icon-shadow" />
                <p className="text-base text-white font-semibold text-shadow">카메라 신호 없음</p>
                <p className="text-xs text-white/80 mt-1 text-shadow-sm">카메라가 꺼져있거나 네트워크 문제입니다</p>
              </div>
            </div>
          )}
          ...
          {selectedCamera.connected && !selectedCamera.enabled && (
            <div className="absolute inset-0 flex items-center justify-center z-[5]">
              <div className="text-center px-6 py-4 rounded-lg border border-white/30 bg-black/40">
                <Video className="h-12 w-12 text-white mx-auto mb-2 icon-shadow" />
                <p className="text-base text-white font-semibold text-shadow">카메라 OFF</p>
              </div>
            </div>
          )}
        </div>
